...
def gerenciar_evento(request):
    if request.method == "GET":
        ...
        #TODO: realizr outros filtros(dT de inicio, termino, intervalo )
        if nome:
            eventos = eventos.filter(nome__contains=nome) #filter lockup
...
def participantes_evento(request, id):
    ...
    if not evento.criador == request.user:
        #raise- gerar um status de erro
        raise Http404('Esse evento não é seu')
    if request.method == "GET":
        participantes = evento.participantes.all()[::2] #filtra o numero de registros a serem exibidos
...
def gerar_certificado(request, id):
    ...
    path_template = os.path.join(settings.BASE_DIR, 'templates/static/evento/img/template_certificados.png')
    ...
    for participante in evento.participantes.all():
        ...
        fonte_nome = ImageFont.truetype(path_fonte, 85)
